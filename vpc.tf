...
resource "aws_lb" "nlb" {
  ...
  load_balancer_type = var.load_balancer_type_nlb
  subnets            = [aws_subnet.public_subnet_1.id,aws_subnet.public_subnet_2.id]

  enable_deletion_protection = var.enable_deletion_protection

  tags = {
    (var.environment_nlb) = var.nlb_tagname
  }
}

### Targrt group information
resource "aws_lb_target_group" "alb_tg_80" {
  name     = var.alb_tg_name_80
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.terraform_vpc.id
}
...
resource "aws_lb_target_group" "alb_tg_443" {
  name     = var.alb_tg_name_443
  port     = 443
  protocol = "HTTPS"
  vpc_id   = aws_vpc.terraform_vpc.id
}

resource "aws_lb_target_group" "alb_tg_port" {
  name     = var.alb_tg_name_port
  port     = var.port_alb_tg
  protocol = var.protocol_alb_tg_port
  vpc_id   = aws_vpc.terraform_vpc.id
}

### Target group information - locals

# locals {
#   name = ["alb_tg_80", "alb_tg_443", "alb_tg_9098"]  
#   port = [ 80, 443, 9098 ]
# }

# resource "aws_lb_target_group" "alb_tg_local" {
#   name     = locals.name
#   port     = locals.port
#   protocol = "HTTP"
#   vpc_id   = aws_vpc.terraform_vpc.id
...
resource "aws_launch_template" "asg_lt" {
  name_prefix   = var.name_prefix_asg_lt
  image_id      = var.image_id_asg_lt
  instance_type = var.instance_type_asg_lt
}
...
resource "aws_autoscaling_group" "asg" {
  ...
  health_check_grace_period = var.health_check_grace_period_asg
  health_check_type         = var.health_check_type_asg
